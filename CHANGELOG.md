...
- **`examples/jev_edge.py`** — "Jev vs the market" strategy: asks the Jev decision model for a YES probability per binary market and buys the side it favors when it disagrees with the midpoint by more than `EDGE`, skipping markets priced outside `[0.05, 0.95]`. Needs `pip install -e "benchmark"`; defaults to the free `opencode/jev-1.13-free` (override with `JEV_MODEL`).
...
### Fixed
- **`pm-trader strategy run examples.x.run` no longer needs `PYTHONPATH=.`** — strategy modules now resolve from the working directory (console scripts don't put the cwd on `sys.path`).
