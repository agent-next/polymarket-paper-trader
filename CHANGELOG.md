...
### Added
- Closed loop (no self-merge): `implement` only on `bot:implement` or `/oc implement` (`deepseek-v4-flash`); after a `GITHUB_TOKEN` push it dispatches the Tests workflow on the branch it just pushed (`gh workflow run Tests --ref <branch>`), failing loudly when there is no pushed ref instead of guessing. `review` stays comment-only; merge gating stays with branch protection.
